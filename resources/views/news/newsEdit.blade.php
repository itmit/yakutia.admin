...
<form class="form-horizontal" method="PATCH" action="{{ route('auth.news.update', ['id' => $id]) }}" enctype="multipart/form-data">
    ...
    <div class="form-group{{ $errors->has('news_head') ? ' has-error' : '' }}">
        ...
        <div class="col-xs-12 col-sm-10">
            <input id="news_head" type="text" class="form-control" name="news_head" value="{{ old('news_head') }} {{ $news->news_head }}" required maxlength="191">
    ...
    <div class="row form-group{{ $errors->has('news_body') ? ' has-error' : '' }}">
        ...
        <div class="col-xs-12 col-sm-10">
            <textarea id="news_body" type="text" class="md-textarea form-control" name="news_body" cols="30" rows="10" maxlength="2000">{{ old('news_body') }} {{ $news->news_body }}</textarea>
    ...
    <div class="form-group{{ $errors->has('news_picture') ? ' has-error' : '' }}">
        ...
        <div class="col-xs-12 col-sm-10">
            <input type="file" name="news_picture" id="news_picture" class="form-control-file" accept="image/*" required value="{{ $news->news_picture }}">
